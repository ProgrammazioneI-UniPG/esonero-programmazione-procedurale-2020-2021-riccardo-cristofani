...
//FUNZIONI:
  ...
  void procedura2(){
    ...
    for (int i = 0; i < Lung_messaggio ; i++) {
      str_chiave[i] = rand()%(126-33+1)+33 ;//(vmax-vmin+1)+vmin. Ho escluso i primi 32 caratteri speciali e il 127esimo.
    }
...
/*leggo messaggio e lo stampo*/
  do{
    ...
    fgets(str_messaggio, ncaratteri ,stdin);//prende da tastiera: ncaratteri-1('/0') cioè 127. carattere invio considerato.
    Lung_messaggio = strlen(str_messaggio);
  }while (Lung_messaggio == 1);
  while (getchar()!='\n');//svuoto il buffer
  printf("\nIl tuo messaggio è:%s\n",str_messaggio);
...
/*scelta della procedura*/
  ...
  if (n_p == 1) {
    printf("\nÈ stata scelta la procedura numero 1\n");
    procedura1();
  }
  else if(n_p == 2){
    printf("\nÈ stata scelta la procedura numero 2\n");
    procedura2();
  }
